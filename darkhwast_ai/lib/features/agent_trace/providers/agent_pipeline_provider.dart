...
final pipelineProvider =
  ...
  return AgentPipelineNotifier(
    ...
    isDemo: ref.watch(demoModeProvider),
...
class AgentPipelineNotifier extends StateNotifier<PipelineState> {
  ...
  void setFilingMeta(
      {required String caseReference, required bool collectiveJoined}) {
  ...
  void updateActionDraft(ActionDraft updated) {
    state = state.copyWith(
      drafter: state.drafter.copyWith(
        result: updated,
      ),
    );
  }
  ...
  Future<void> runPhase1(File? documentImage,
      {String? text, bool autoDetect = false}) async {
    ...
    try {
      ...
      state = state.copyWith(
          docIntel: AgentState.loading("Document scan ho rahi hai..."));
      await Future.delayed(delay);

      final scenario = isDemo ? demoScenario : 'electricity_bill';
      ...
      if (isDemo) {
        ...
        if (extractedText.isEmpty && documentImage != null) {
          extractedText = await ocr.extractText(documentImage);
        } else if (extractedText.isEmpty) {
          extractedText = "Mock analysis for demo purposes";
        }
      ...
      state = state.copyWith(
        docIntel: AgentState.complete(
          ...
          message:
              "Detected ${docEntity.type.displayName} from ${docEntity.authority}. "
              "Extracted ${docEntity.amounts.length} charge lines and ${docEntity.deadlines.length} date signals.",
      ...
      final List<DeadlineAlert> deadlines = docEntity.deadlines
          .map((d) => DeadlineAlert(
                ...
                urgencyLevel: (d['daysRemaining'] != null &&
      ...
      final hasUrgent =
          deadlines.any((d) => d.urgencyLevel == UrgencyLevel.high);
      ...
      if (deadlines.isEmpty) {
        ...
        urgencyMsg = "⚠️ URGENT: ${d.label} — ${d.daysRemaining} din baaki! "
      ...
      if (autoDetect) {
        ...
        state = state.copyWith(
          overallStatus: PipelineStatus.awaitingIntent,
        );
      }
    } catch (e) {
      state = state.copyWith(
  ...
  Future<void> _runPhase2(
    ...
    try {
      ...
      if (isDemo) {
        ...
        final law = await lawKnowledge.getLawKnowledge(
      ...
      state = state.copyWith(
        drafter: AgentState.loading(
          _getAgent4LoadingMessage(intent.intent),
        ),
      );
      await Future.delayed(delay);
      ...
      ActionDraft draft;
      if (isDemo) {
        final mockDraft = await MockResponseService.getMockDraft(scenario);
        // Wrap mock in intent-aware ActionDraft
        draft = ActionDraft(
          urduDraft: mockDraft.urduDraft,
          englishDraft: mockDraft.englishDraft,
          subject: mockDraft.subject,
          submissionAuthority: mockDraft.submissionAuthority,
          submissionPortal: mockDraft.submissionPortal,
          estimatedResponseDays: mockDraft.estimatedResponseDays,
          actionType: intent.intent,
          actionSummary: _getActionSummary(intent.intent, rightsAnalysis),
        );
      } else {
        draft = await gemini.draftAction(
          rightsAnalysis,
          docEntity,
          intent,
        );
      ...
      final patternMessage = finalCluster == null
